...
class PlaywrightMockingAddon {
    ...
    async mockJson<T extends JsonValue>(
        url: MockUrl,
        response: JsonMockResponse<T>,
    ): Promise<void> {
        await this.register(url, async (route) =>
            this.fulfillJson(route, response),
        );
    }

    async mockRestEndpoint<T extends JsonValue>(
        url: MockUrl,
        response: RestMockResponse<T>,
    ): Promise<void> {
        await this.register(url, async (route) => {
            if (
                response.method &&
                route.request().method() !== response.method.toUpperCase()
            ) {
    ...
    async mockGraphQLOperation<T extends JsonValue>(
        url: MockUrl,
        response: GraphQLMockResponse<T>,
    ): Promise<void> {
        await this.register(url, async (route) => {
            ...
            const operationName =
                ...
                this.operationNameFromQuery(requestBody?.query);
    ...
    async mockGraphQLError(
        ...
        await this.mockGraphQLOperation(url, {
            operationName,
            status,
    ...
    async clearMocks(): Promise<void> {
        await Promise.all(
            this.registeredRoutes.map(({ url, handler }) =>
    ...
    private async fulfillJson<T extends JsonValue>(
        route: Route,
        response: JsonMockResponse<T>,
    ): Promise<void> {
        await route.fulfill({
