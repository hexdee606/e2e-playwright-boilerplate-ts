...
Given(/^the mocked REST product endpoint returns a server error$/, async function ({page}) {
...
Then(/^the mocked REST response status should be (\d+)$/, async function ({}, status: number) {
    expect(this.response.status).toBe(Number(status));
});

Then(/^the mocked REST product should match its contract$/, async function ({}) {
    mockedProductResponseSchema.parse(this.response.data);
});

Then(/^the mocked REST response should contain an error message$/, async function ({}) {
