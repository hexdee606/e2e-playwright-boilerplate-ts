...
import {
    mockedGraphQLErrorResponseSchema,
    mockedGraphQLPostResponseSchema,
} from "../contracts/mock_graphql_contract";
...
Given(
    /^the mocked GraphQL post query returns an error$/,
    async function ({ page }) {
...
Then(
    /^the mocked GraphQL response status should be (\d+)$/,
...
Then(
    /^the mocked GraphQL response should match its contract$/,
    async function ({}) {
...
Then(
    /^the mocked GraphQL response should contain errors$/,
    async function ({}) {
